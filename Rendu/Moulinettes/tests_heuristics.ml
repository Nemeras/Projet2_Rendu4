...
let series file med j =
	med.(0).(j) <- test file " -cl -rand" ;
	med.(1).(j) <- test file " -cl -moms" ;
	med.(2).(j) <- test file " -cl -dlis" ;
	med.(3).(j) <- test file " -vsids" ;
	med.(4).(j) <- test file " -cl -wl -rand" ;
	med.(5).(j) <- test file " -cl -wl -moms" ;
	med.(6).(j) <- test file " -cl -wl -dlis" ;
	med.(7).(j) <- test file " -wl -vsids" ;
	med.(8).(j) <- test file " -cl" ;
	med.(9).(j) <- test file " -cl -wl"
...
let tests k n nbr =
	let res = Array.make_matrix nbr 10 0. in
	...
	for i = 1 to nbr do
		...
		let med = Array.make_matrix 10 10 0. in
		for j = 1 to 5 do
...
let dat res n nbr =
	...
	Printf.fprintf f "argument RAND MOMS DLIS VSIDS RAND-WL MOMS-WL DLIS-WL VSIDS-WL EMPTY EMPTY-WL\n" ;
...
let generate () =
	let _ = Unix.system "gnuplot -persist Moulinettes/plot_heuristics.p" in
	Unix.system "gnuplot -persist Moulinettes/plot_heuristics_wl.p"
...
let _ =
	...
	Arg.parse options (fun s -> n := int_of_string s) "Ce programme génère une courbe gnuplot correspondant à des tests sur les différentes heuristiques de DPLL" ;
